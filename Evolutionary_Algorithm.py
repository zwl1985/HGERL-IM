...
class EA:
    def __init__(self, pop_size):
        self.current_gen = 0  # 记录当前的进化代数
        self.pop_size = pop_size  # 设置种群的规模（即每代个体数量）
        self.elite_fraction = 0.2  # 设置精英保留比例
        self.num_elitists = int(
            self.elite_fraction * self.pop_size)  # 计算每代保留的精英个体数量
    ...
    def selection_tournament(self, 
                             # 个体的排名列表，索引对应个体，值表示适应度排名
                             index_rank, 
                             # 需要选择的子代数量
                             num_offsprings, 
                             # 锦标赛的大小，即每次选择中随机选取的个体数量
                             tournament_size):
